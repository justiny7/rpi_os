...
/* Peripheral base matching GPIO_BASE (0x20200000); EMMC at base + 0x300000 = 0x20300000 */
#define MMIO_BASE 0x20000000

/* GPIO registers for SD card pin function select and event detect (GPFSEL4/5, GPHEN1) */
#define GPFSEL4   ((volatile unsigned int *)(GPFSEL_BASE + 0x10))
#define GPFSEL5   ((volatile unsigned int *)(GPFSEL_BASE + 0x14))
...
int sd_int(unsigned int mask)
{
    ...
    int cnt = 1000000; while(!(*EMMC_INTERRUPT & m) && cnt--) sys_timer_delay_ms(1);
    r=*EMMC_INTERRUPT;
...
int sd_cmd(unsigned int code, unsigned int arg)
{
    ...
    *EMMC_INTERRUPT=*EMMC_INTERRUPT; *EMMC_ARG1=arg; *EMMC_CMDTM=code;
    ...
    if(code==CMD_SEND_IF_COND) return (unsigned int)r==arg? SD_OK : SD_ERROR; else
...
int sd_clk(unsigned int f)
{
    ...
    int cnt = 100000;
    while((*EMMC_STATUS & (SR_CMD_INHIBIT|SR_DAT_INHIBIT)) && cnt--) sys_timer_delay_ms(1);
    if(cnt<=0) {
        uart_puts("ERROR: timeout waiting for inhibit flag\n");
        return SD_ERROR;
    }

    *EMMC_CONTROL1 &= ~C1_CLK_EN; sys_timer_delay_ms(10);
    x=c-1; if(!x) s=0; else {
    ...
    d=(((d&0x0ff)<<8)|h);
    *EMMC_CONTROL1=(*EMMC_CONTROL1&0xffff003f)|d; sys_timer_delay_ms(10);
    *EMMC_CONTROL1 |= C1_CLK_EN; sys_timer_delay_ms(10);
    cnt=10000; while(!(*EMMC_CONTROL1 & C1_CLK_STABLE) && cnt--) sys_timer_delay_ms(10);
    if(cnt<=0) {
...
{
    long r,cnt,ccs=0;
    // GPIO pull-up for SD pins (47=CD, 48=CLK, 49=CMD, 50-53=DAT0-3)
    gpio_set_pull((Pin){.p_num = 47}, GPIO_PULL_UP);
    gpio_set_pull((Pin){.p_num = 48}, GPIO_PULL_UP);
    gpio_set_pull((Pin){.p_num = 49}, GPIO_PULL_UP);
    gpio_set_pull((Pin){.p_num = 50}, GPIO_PULL_UP);
    gpio_set_pull((Pin){.p_num = 51}, GPIO_PULL_UP);
    gpio_set_pull((Pin){.p_num = 52}, GPIO_PULL_UP);
    gpio_set_pull((Pin){.p_num = 53}, GPIO_PULL_UP);

    // GPIO_CD (pin 47) as input, enable high detect
    r=*GPFSEL4; r&=~(7<<(7*3)); *GPFSEL4=r;
    r=*GPHEN1; r|=1<<15; *GPHEN1=r;

    // GPIO_CLK, GPIO_CMD (pins 48, 49) as ALT3
    r=*GPFSEL4; r|=(7<<(8*3))|(7<<(9*3)); *GPFSEL4=r;

    // GPIO_DAT0-3 (pins 50-53) as ALT3
    ...
    *EMMC_CONTROL0 = 0; *EMMC_CONTROL1 |= C1_SRST_HC;
    cnt=10000; do{sys_timer_delay_ms(10);} while( (*EMMC_CONTROL1 & C1_SRST_HC) && cnt-- );
    if(cnt<=0) {
